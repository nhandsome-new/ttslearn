...
class Tacotron2(nn.Module):
    ...
    def inference(self, seq):
        ...
        in_lens = torch.tensor(seq.shape[1], dtype=torch.long, device=seq.device).unsqueeze(-1)
...
if __name__ == '__main__':
    import torch
    taco = wrapper_model(Tacotron2())
    
    taco.eval()
    
    input_names = ['tokenized_text']
    output_names = ['mel_out', 'fined_mel_out', 'logits', 'att_ws']
    sample_inputs = torch.randint(1,52,(3,))
